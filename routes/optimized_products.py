# routes/optimized_products.py - Optimized Product Routes with Caching
...
from routes.caching import cache  # Cache entegrasyonu
...
async def get_user_products_optimized(
    ...
    """Get user products with pagination, optimization, and caching."""
    
    # Check cache first
    cache_key = f"products_{current_user.uid}_{limit}_{offset}_{include_photos}_{sort_by}_{sort_order}"
    cached_result = await cache.get(cache_key) # Düzeltme: cache_key doğrudan kullanıldı
    if cached_result:
        logger.info(f"Cache hit for user products: {current_user.uid}")
        return [ProductListResponse.model_validate(item) for item in cached_result]
    ...
    try:
        ...
        # Cache the result
        cache_data = [product.model_dump() for product in final_products]
        # Düzeltilmiş satır: cache_key, cache_data'dan önce gelmeli
        await cache.set(cache_key, cache_data, ttl_seconds=1800)  # 30 min cache
